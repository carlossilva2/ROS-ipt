...
class GreenTpid():
    ...
    def __init__(self):
        rospy.init_node("greenT_pid_vel_left")
        ...
        self.Ki = rospy.get_param('~Ki',0.35)
        self.Kd = rospy.get_param('~Kd',1)
        self.out_min = rospy.get_param('~out_min',-204)
        self.out_max = rospy.get_param('~out_max',204)
        self.rate = rospy.get_param('~rate',30)
        self.rolling_pts = rospy.get_param('~rolling_pts',2)
        self.timeout_ticks = rospy.get_param('~timeout_ticks',10)
        self.ticks_per_meter = rospy.get_param('ticks_meter', int(562/(2*math.pi*0.035)))
        ...
        rospy.Subscriber("wheel_vtarget", Int16, self.targetCallback)
        self.pub_motor = rospy.Publisher('motor_cmd',Int16, queue_size=10) 
        self.pub_vel = rospy.Publisher('wheel_vel', Int16, queue_size=10)
